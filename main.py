...
# 행성에 도착했을 때 잃는 연료와 얻는 자원을 구하는 함수
...
# 배경사진 출력
def background(bg_img, bg_x, bg_y):
...
def gameScreen(img_act):
    ...
    # 특정부분의 코드가 프레임마다 실행되는 것을 방지하기 위한 플래그
    isBlocked = False                   
    ...
    while not gameexit:
        ...
        # 만약 현재 시간이 endTime보다 작고 msgChance가 빈 문자열일경우 출력
        if pg.time.get_ticks() <= endTime and msgChance != '':    
            printText2(msgChance, "YELLOW", 50, (370, 150))
        else:
            msgChance = '' 
        
        # 만약 현재 시간이 endTime보다 작고 msgCommunity가 빈 문자열일경우 출력
        ...
        if diceCnt in chanceTile:
            ...
            # timerWait이 None이 아니고 일정시간이 지났을경우
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key== pg.K_SPACE and spaceBlock == False:
                # Dice객체 안 숫자를 랜덤하게 바꾸는 함수
                clickDice.rollDice()    
                pressSpaceBlock = True
                # Dice객체 안 end_time을 설정하는 함수 (단위 ms)
                clickDice.timer(1300)
                # 텍스트 출력 타이머
